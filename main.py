...
from utils.settings import get_settings, Settings
...
def main():
    ...
    github_data_df['language'] = github_data_df['language'].apply(lambda s: s.strip() if isinstance(s, str) else s)
...
def clean_dataset(df: pd.DataFrame) -> pd.DataFrame:
    """
    helper for things that needs to be done on both datasets.
    """
    ret = df.copy()
